...
def sentence1() -> Expr:
    ...
    s1 = Expr('|', 'A', 'B')
    s2 = Expr('<=>', Expr('~', 'A'), Expr('|', Expr('~', 'B'), 'C'))
    s3 = Expr('|', Expr('~', 'A'), Expr('~', 'B'), 'C')
    
    # Conjoin the sentences
    return Expr('&', s1, s2, s3)
...
def sentence2() -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    s1 = Expr('<=>', 'C', Expr('|', 'B', 'D'))
    s2 = Expr('>>', 'A', Expr('&', Expr('~', 'B'), Expr('~', 'D')))
    s3 = Expr('>>', Expr('~', Expr('&', 'B', Expr('~', 'C'))), 'A')
    s4 = Expr('>>', Expr('~', 'D'), 'C')
    return Expr('&', s1, s2, s3, s4)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    list = []
    for each in itertools.combinations(literals, 2):
        l1 = ~each[0] | ~each[1]
        list.append(l1)
    return conjoin(list)
    util.raiseNotDefined()
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x, y in non_outer_wall_coords:
        listP.append(PropSymbolExpr(pacman_str, x, y, time=t))
    pacphysics_sentences.append(exactlyOne(listP))
    
    listD = []
    for dir in DIRECTIONS:
        listD.append(PropSymbolExpr(dir, time=t))
    pacphysics_sentences.append(exactlyOne(listD))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    axiom0 = pacphysicsAxioms(0, all_coords, non_outer_wall_coords, walls_grid, successorAxioms=allLegalSuccessorAxioms)
    axiom1 = pacphysicsAxioms(1, all_coords, non_outer_wall_coords, walls_grid, successorAxioms=allLegalSuccessorAxioms)
    KB.append(axiom0)
    KB.append(axiom1)
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            listC.append(PropSymbolExpr(pacman_str, x, y, time=t))
        KB.append(exactlyOne(listC))
        
        listD = []
        for dir in DIRECTIONS:
            listD.append(PropSymbolExpr(dir, time=t))
        KB.append(exactlyOne(listD))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for action in actions:
                listA.append(PropSymbolExpr(action, time=t))
        KB.append(exactlyOne(listA))
...
def addToKB(KB, t, agent, all_coords, non_outer_wall_coords, walls_grid, sAxioms, SuccessorAxion, PerceptRules):
    KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid, sAxioms, SuccessorAxion))
    KB.append(PropSymbolExpr(agent.actions[t], time=t))
    KB.append(PerceptRules(t, agent.getPercepts()))

def findLoc(KB, t, non_outer_wall_coords, possible_locations):
    for x, y in non_outer_wall_coords:
...
def findProv(KB, known_map, non_outer_wall_coords):
    for x, y in non_outer_wall_coords:
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        addToKB(KB, t, agent, all_coords, non_outer_wall_coords, walls_grid, sensorAxioms, allLegalSuccessorAxioms, fourBitPerceptRules)
    
        findLoc(KB, t, non_outer_wall_coords, possible_locations)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        addToKB(KB, t, agent, all_coords, non_outer_wall_coords, known_map, sensorAxioms, allLegalSuccessorAxioms, fourBitPerceptRules)
        
        findProv(KB, known_map, non_outer_wall_coords)
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        possible_locations = []
        addToKB(KB, t, agent, all_coords, non_outer_wall_coords, known_map, SLAMSensorAxioms, SLAMSuccessorAxioms, numAdjWallsPerceptRules)
        findLoc(KB, t, non_outer_wall_coords, possible_locations)
        findProv(KB, known_map, non_outer_wall_coords)
